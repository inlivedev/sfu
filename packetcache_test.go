	...
	for i, unsorted := range unsortedNumbers {
		if unsorted < 65531 && unsorted > 65525 {
			if unsorted == 65526 || unsorted == 65527 {
				tid, sid, err = caches.GetDecision(unsorted, currentSID, currentTID, KEEPLAYER)
				if i > 1 {
					require.Equal(t, err, ErrDuplicate)
				}
			} else if unsorted == 65531 {
				tid, sid, err = caches.GetDecision(unsorted, currentSID, currentTID, SCALEUPLAYER)
			...
			caches.Add(unsorted, uint8(3), uint8(3))
		} else {
